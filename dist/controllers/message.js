...
const convertCurrencyHandler = async (msg, match) => {
  ...
  if (convertedPrice) {
    ...
    rootIndex.bot.sendMessage(chatId, `Неправильный формат сообщения. Пример: /convert 0.05 btc`);
  }
};
...
const newMessageHandler = async msg => {
  if (typeof msg.text !== "undefined") {
    ...
    if (rootIndex.availableСurrencies?.find(({
      slug
    }) => slug === messageText)) {
      console.log("a");
...
const newChatMember = msg => {
  const chatId = msg.chat.id;
  rootIndex.bot.sendPhoto(chatId, "https://avatars.mds.yandex.net/get-images-cbir/516543/hWbMF_mtRBHJcXd17iP-5A4300/ocr", {
